...
RUN apt update -y && \
    ...
    opencv-python opencv-contrib-python pytorch-msssim thop einops nvidia-pyindex tensorrt==8.6.0 kornia \
...
RUN wget https://github.com/styler00dollar/ffmpeg-static-arch-docker/releases/download/29d-03m-23y-py310/ffmpeg && \
...
RUN pip install scikit-build && git clone --recursive https://github.com/opencv/opencv-python.git && cd opencv-python && git submodule update --init --recursive && git submodule update --remote --merge && \
    CMAKE_ARGS="-DOPENCV_EXTRA_MODULES_PATH=/workspace/opencv-python/opencv_contrib/modules -D BUILD_TIFF=ON -D BUILD_opencv_java=OFF -D WITH_CUDA=ON -D WITH_OPENGL=ON -D WITH_OPENCL=ON -D WITH_IPP=ON -D WITH_TBB=ON -D WITH_EIGEN=ON -D WITH_V4L=OFF  -D BUILD_TESTS=OFF -D BUILD_PERF_TESTS=OFF -D CMAKE_BUILD_TYPE=RELEASE -D OPENCV_FFMPEG_USE_FIND_PACKAGE=ON -D BUILD_SHARED_LIBS=OFF -D CUDA_ARCH_BIN=7.5,8.0,8.6,8.9 -D CMAKE_BUILD_TYPE=RELEASE" \
    ENABLE_CONTRIB=1 MAKEFLAGS="-j$(nproc)" python setup.py bdist_wheel --verbose && cd dist && pip install *.whl --force-reinstall && cd .. && cd .. && rm -rf opencv-python
