...
def sepconv_model(clip: vs.VideoNode) -> vs.VideoNode:
    ...
    clip1 = clip0.std.DuplicateFrames(frames=clip0.num_frames - 1).std.DeleteFrames(
        frames=0
    )
...
def frame_to_tensor(f: vs.VideoFrame) -> torch.Tensor:
    arr = np.stack(
        [
            np.asarray(f.get_read_array(plane) if vs_api_below4 else f[plane])
...
def tensor_to_frame(t: torch.Tensor, f: vs.VideoFrame) -> vs.VideoFrame:
    ...
    for plane in range(fout.format.num_planes):
        np.copyto(
            np.asarray(fout.get_write_array(plane) if vs_api_below4 else fout[plane]),
            arr[plane, :, :],
        )
    return fout
