...
def paired_walk_dir(s_path=None, t_path=None, o=None, algo=None, regrain=None, histo=None, blending=None, rep=None, threads=8):
    ...
    with Pool(threads) as p:
        r = list(tqdm(p.starmap(apply_transfer, zip(source_paths, target_paths, repeat(o), repeat(algo), repeat(regrain), \
            repeat(histo), repeat(blending), repeat(rep))), total=len(source_paths)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-threads', type=int, default=8, required=False, help='Amount of threads for folder processing.')
    ...
    #check if both paths are a directory, must be paired images
        ...
        paired_walk_dir(s_path=s, t_path=t, o=o, algo=algo, regrain=regrain, histo=histo, blending=blending, rep=rep, threads=threads)
    else:
