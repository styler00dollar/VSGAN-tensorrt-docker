...
# frames to delete
def processInfo():
    with open("infos_running.txt",'r') as f:
...
def newTSgen(tsv2o):
    ts_new=list()
    outfile=open("tsv2nX8.txt",'w',encoding='utf-8')
...
clip = core.rife.RIFE(clip,model=11,sc=True,multiplier=8)
clip = core.resize.Bicubic(clip,format=vs.YUV420P10,matrix=1,dither_type='error_diffusion')
clip = core.vfrtocfr.VFRToCFR(clip,r"tsv2nX8.txt",192000,1001,True) # 24fps * 8
