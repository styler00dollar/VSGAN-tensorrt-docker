...
class Network(torch.nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        tenOne = torch.nn.functional.pad(input=x1, pad=[0, intPadr, 0, intPadb], mode='replicate')
        tenTwo = torch.nn.functional.pad(input=x2, pad=[0, intPadr, 0, intPadb], mode='replicate')
        ####
        ...
        tenOut = sepconv_func.apply(tenOne, tenVerone, tenHorone) + sepconv_func.apply(tenTwo, tenVertwo, tenHortwo)
...
def sepconv_model(clip: vs.VideoNode) -> vs.VideoNode:
