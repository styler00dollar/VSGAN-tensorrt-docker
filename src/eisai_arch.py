...
class I:
    ...
    def pil(self, mode=None):
        if self.dtype == "pil":
            ...
            ans = Image.fromarray(
                (data.transpose(1, 2, 0) if data.shape[0] != 1 else data[0]).astype(
                    np.uint8
                )
            )
    ...
    # text
    def text(
        self, text, pos, s=12, anchor="tl", c="m", bg="k", spacing=None, padding=0
    ):
        t = itext(
            text,
            s=s,
...
def cropbox_points(pts, from_corner, from_size, to_size):
    ...
    sf = np.asarray([ts[0] / fs[0], ts[1] / fs[1]])[
        None,
    ]
    return (pts - fc) * sf
...
def iblank(size, c=(0, 0, 0, 1)):
    ...
    if c is None:
        c = "a"
    c = c255(c)
    return I(Image.fromarray(np.asarray(c, dtype=np.uint8)[None, None])).resize(
        size, resample="nearest"
    )
